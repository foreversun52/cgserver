           ...
           LEFT JOIN
             (SELECT DISTINCT device_id AS p_deviceid,
              ...
              WHERE device_id <> ''
                ...
                AND post_back_day = {day})) ) AS p ON c.bid_id = p.bid_id
           WHERE bid_year= {year}
             AND bid_month = {month}
             AND bid_day  = {day}
           GROUP BY sub_type,
...
def dayDataContrast(dataFrame, dateList):
    ...
    if hourList[1] == 1:
        ...
        dayReport = "全局:昨天与前天的对比:\n"
        for k, v in dayData[0].items():
            flag, status, diff = judge(float(dayData[0].get(k)), float(dayData[1].get(k)))
            dayReport += f'{k}:昨天{dayData[1].get(k)}，前天:{dayData[0].get(k)},{status}了{round(diff * 100, 2)}%\n'
        dayReport += "-----------------------------------\n"
        ...
        for dayBidCoutry in dayBidCoutries:
            ...
            for k, v in dayCountryData[0].items():
                ...
                report += f'{k}:昨天{dayCountryData[1].get(k)}，前天:{dayCountryData[0].get(k)},{status}了{round(diff * 100, 2)}%\n'
        ...
        for dayAdxId in dayAdxIds:
            ...
            for k, v in dayAdxData[0].items():
                ...
                report += f'{k}:昨天{dayAdxData[1].get(k)}，前天:{dayAdxData[0].get(k)},{status}了{round(diff * 100, 2)}%\n'
...
def hourDataContrast(dataFrame, dateList):
    ...
    for k, v in hourAllData[0].items():
        ...
        if not flag:
            report += f'{k}-ALL-ALL:UTC-{hourList[1]}:{hourAllData[1].get(k)}，昨天UTC-{hourList[1]}:{hourAllData[0].get(k)},{status}了{round(diff * 100, 2)}%\n'
            isAdd = True
    if isAdd:
    ...
    hourRefineCountryReport = "小时级细化指标Country对比昨日\n"
    # 细化指标Country对比昨日
    ...
    hourRefineAdxReport = "小时级细化指标Adx对比昨日\n"
    ...
    if hourList[1] >=1 and hourList[1] <= 16:
